...
/** 用户画廊：filesDir/gallery + gallery.json */
object UserGalleryStore {
    private const val INDEX = "gallery.json"

    data class Entry(val id: String, val title: String, val filename: String)

    private fun dir(ctx: Context) = File(ctx.filesDir, "gallery").also { it.mkdirs() }
